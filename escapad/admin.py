...
class RepositoryAdmin(admin.ModelAdmin):
    ...
    list_display = ('git_url', 'show_feedback',
                    'repo_synced', 'default_branch',
                    'last_compiled',  'build_url',
                    'site_url', 'build_and_zip')

    # Form method we use to process some custom chekings. See forms.py
    form = RepositoryForm

    def get_readonly_fields(self, request, obj):
        """Method in charge of populating the readonly_fields from the list
        given in above attribute We override this method just to make
        the request object available in the methods defining the
        custom read-only fields below, specifically build_url_long and
        site_url_long
        """
        self.request = request
        # fields that won't be editable. Just remove one to make it editable
        readonly_fields = ('git_username', 'git_name',
                           'repo_synced', 'last_compiled',
                           'provider', 'site_url_long',
                           'build_url_long', 'slug')
        if obj:
    ...
    # ========  custom fields  =============#
    # Below are the method used to define the custom
    # fields, i.e those not defined in model.py
    ...
    def build_url_long(self, obj):
        ...
        if obj.slug:
            url = self.request.build_absolute_uri(reverse('build_repo',
                                                          args=(obj.slug,)))
    ...
    def site_url_long(self, obj):
        if obj.slug:
            url = self.request.build_absolute_uri(reverse('visit_site',
                                                          args=(obj.slug,)))
            return '<a href="%s" target="_blank">%s<a>' % (url, url)
        else:
            return ''
